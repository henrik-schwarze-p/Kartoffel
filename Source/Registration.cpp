...
void pregister(RCConfig* rc) {
    pgregister(&rc->on, 1);
    pgregister(&rc->port, 1);
    pgregister(&rc->codeOn, 4);
    pgregister(&rc->codeOff, 4);
    pgregister(&rc->protocol, 1);
    pgregister(&rc->type, 1);
    pgregister(&rc->group, 1);
    pgregister(&rc->number, 1);
}

...
int   boardIndex = 0;
char* boardLinesText[boardMaxLines];
...
void initBoard() {
    boardIndex = 0;
    boardLinesText[0] = (char*)malloc(10000);
    strcpy(boardLinesText[0], "");
}

void boardPrint(const char* c) {
    printf("%s", c);
    strncat(boardLinesText[boardIndex], c, 999);
    boardTextRepaint();
}

void boardPrintln() {
    printf("\n");
    if (boardIndex + 1 == boardMaxLines) {
        for (int i = 0; i < boardMaxLines - 1; i++)
            strcpy(boardLinesText[i], boardLinesText[i + 1]);
    } else {
        boardIndex++;
        boardLinesText[boardIndex] = (char*)malloc(1000);
    }
    strcpy(boardLinesText[boardIndex], "");
    boardTextRepaint();
}
...
void boardPrint(float f) {
    boardPrint((int)f);
    boardPrint(".");
    int dec = (int)(f * 1000 - (int)f * 1000);
    int u = (int)dec;
    boardPrint(u / 100);
    boardPrint((u / 10) % 10);
    boardPrint(u % 10);
...
void boardPrint(int number, int digits) {
    char buffer[1000];
    if (digits == 2) {
        snprintf(buffer, 1000, "%02X", number);
    }
    if (digits == 4) {
        snprintf(buffer, 1000, "%04X", number);
    }
    boardPrint(buffer);
}

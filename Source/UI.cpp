...
void drawToolbar(int forceRepaint) {
    ...
    for (int i = 0; i < toolbarCount; i++) {
        ...
        if (toolbarButtons[i].enabled)
            setColor(colorWhite);
        if (paint) {
            drawString(label, x + w / 2 - stringWidth(label, t.fromLabel, t.toLabel) / 2, 210,
                       toolbarButtons[i].fromLabel, toolbarButtons[i].toLabel);
        }
        x += w;
    }
...
void toolbarAddScreen(toolbarButtonSize mini, int isEnabled, const char* label, screen screen, int first, int last) {
    if (!goToScreenLock())
        fatalError(22, 0);
    if (toolbarNextAddResets) {
        toolbarCount = 0;
...
void toolbarAddScreen(const char* label, screen screen) {
    toolbarAddScreen(standard, isenabled, label, screen);
}

void toolbarAddScreen(int isEnabled, const char* label, screen screen) {
    toolbarAddScreen(standard, isEnabled, label, screen);
}
...
void toolbarAddScreen(toolbarButtonSize mini, int isEnabled, const char* label, screen screen) {
    toolbarAddScreen(mini, isEnabled, label, screen, 0, stringLen(label) - 1);
}

void toolbarAddHome() {
    toolbarAddScreen(mini, 1, MINI_ICON_HOME, desktop::startScreen);
}

void toolbarAddScreenBack(screen back) {
    toolbarAddScreen(mini, 1, PSTR("<<"), back);
}
...
void drawMessage() {
    if (!_message)
        return;
    // repaintScreen(a, clear * DRAWSCREEN_CLEAR_MIDDLE);
    resetButtons();
    setBigFont();
    setPrintX(2 * margin);
    ...
    int count = 0;
    int widths[10];
    measureText(_message, widths, &count);
    ...
    for (int i = 0; i < count; i++) {
        ...
        drawCenteredStringPart(_message, p, widths[i], y);
        y += 18;
    }
...
void repaintScreen(screen screen) {
    if (goToScreenLock())
        fatalError(24, 0);
    ...
    setColor(colorBG);
    if (repaintState() & REPAINT_MENUBAR) {
        fillRect(0, 0, horizontalResolution, menuBarHeight);
        boardPrintln("Repainting MENUBAR");
    }
    if (repaintState() & REPAINT_CLEAN_MIDDLE) {
        cleanMiddle();
    ...
    setStandardFont();
    setScale(1);
    boardPrintln("Repainting SCREEN");
    screen();
    setScale(1);
    setMargins(margin, margin);
    setStandardFont();
...
void goToScreen(screen s) {
    ...
    if (getForegroundInstance() == getActiveInstance() || !getForegroundInstance()) {
        ...
        setEnterScreen(s, getActiveInstance());
    }
}
